...
class RummyCustomerSupportV2 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  updateDataHistory(value) {
    this.setState({ dataHistory: value });
  }
  ...
  getTurnDetails(roundId, joker) {
    ...
    this.props.actions.getTurnsByRoundId(data).then(() => {
      if (this.props.getTurnsByRoundIdResponse) {
        if (this.props.getTurnsByRoundIdResponse.turns) {
          this.setState({
            turnDetailList: [...this.props.getTurnsByRoundIdResponse.turns],
  ...
  getMoneyLost(record) {
    ...
    if (record.tableDetails.pointValue) {
      return parseInt(pointsLoss * record.tableDetails.pointValue);
    } else {
      return record.tableDetails.buyIn;
    }
  }
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (this.state.searchCriteria === 'MOBILE_NUMBER') {
          ...
          this.props.actions.getRoundsPlayedByMobile(data).then(() => {
            if (
              this.props.getRoundsPlayedByMobileResponse &&
              this.props.getRoundsPlayedByMobileResponse.rounds
            ) {
              this.setState({
                allRoundsList: [
                  ...this.props.getRoundsPlayedByMobileResponse.rounds
      ...
      <React.Fragment>
        <Form onSubmit={this.handleSubmit}>
          <Card bordered={false} title="User Details">
            <FormItem {...formItemLayout} label={'Search Criteria'}>
              {getFieldDecorator('searchCriteria', {
                ...
                >
                  ...
                  <Radio.Button disabled value={'TABLE_ID'}>
                    Table ID
                  </Radio.Button>
                </Radio.Group>
              )}
            ...
            <FormItem {...formItemLayout} label={'Data search category'}>
              {getFieldDecorator('dataHistory', {
                rules: [
                  {
                    required: true,
                    message: 'Please select an option',
                ...
                initialValue: this.state.dataHistory
              })(
                <Radio.Group
                  onChange={e => this.updateDataHistory(e.target.value)}
                  size="small"
                  buttonStyle="solid"
                >
                  <Radio.Button value={'OLD_DATA'}>Old Data</Radio.Button>
                  <Radio.Button value={'NEW_DATA'}>New Data</Radio.Button>
                </Radio.Group>
              )}
            </FormItem>
            <Button
...
function mapStateToProps(state, ownProps) {
  return {
    ...
    getRoundsPlayedByMobileResponse:
      state.rummyCustomer.getRoundsPlayedByMobileResponse,
    getTurnsByRoundIdResponse: state.rummyCustomer.getTurnsByRoundIdResponse
  };
}
...
const RummyCustomerSupportV2Form = Form.create()(RummyCustomerSupportV2);
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(RummyCustomerSupportV2Form);
