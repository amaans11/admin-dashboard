...
class CommonUploader extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      uploaded: false,
  ...
  handleBeforeUpload = file => {
    ...
    let data = {
      ...
      fileName: fileName,
      contentDisposition:
        extension.toLowerCase().includes('zip') ||
        extension.toLowerCase().includes('apk')
    ...
    this.props.actions.getCommonUploadUrl(data).then(() => {
      if (this.props.getCommonUploadUrlResponse) {
        const {
          uploadUrl,
          error,
          object
        } = this.props.getCommonUploadUrlResponse;
        if (!error) {
          fetch(uploadUrl, {
            body: file,
            method: 'PUT'
...
export default connect(mapStateToProps, mapDispatchToProps)(CommonUploader);
