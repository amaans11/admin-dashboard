...
// type SpecialRewardTable ={}
const FormItem = Form.Item;
const rewardData = [];
function hasErrors(fieldsError) {
  return Object.keys(fieldsError).some(field => fieldsError[field]);
}
class SpecialRewardTable extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    if (
      ...
      this.props.tournament.cloneConfig.specialRewards
    ) {
      const rankRanges = this.props.tournament.cloneConfig.specialRewards
        .rankRanges;
      let result = [];
      if (rankRanges && rankRanges.length > 0) {
        rankRanges.map(range => {
          result.push({
            start: range['start'],
            end: range['end'],
            cash: range['moneyCash']
          });
        });
      }
      this.setState({
        rewardData: result,
        tableCash: this.props.tournament.cloneConfig.specialRewards
          .moneyTotalCash
          ? this.props.tournament.cloneConfig.specialRewards.moneyTotalCash
          : 0,
        tableTokens: this.props.tournament.cloneConfig.specialRewards
          .totalTokens
          ? this.props.tournament.cloneConfig.specialRewards.totalTokens
          : 0,
        startRankVal:
          this.props.tournament.cloneConfig.specialRewards.rankRanges[
            this.props.tournament.cloneConfig.specialRewards.rankRanges.length -
              1
          ].end + 1
      });
      this.props.form.setFieldsValue({
        ...
        extReward: this.props.tournament.cloneConfig.specialRewards.rankRanges[
          this.props.tournament.cloneConfig.specialRewards.rankRanges.length - 1
        ].extReward
      });
    }
  }
...
//   return {
//     actions: bindActionCreators(SpecialRewardTableActions, dispatch)
//   };
// }

const SpecialRewardTableForm = Form.create()(SpecialRewardTable);
export default connect(
  mapStateToProps
  // mapDispatchToProps
)(SpecialRewardTableForm);
// export default SpecialRewardTableForm;
