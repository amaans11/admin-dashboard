...
class UploadSegment extends React.Component {
  ...
  render() {
    ...
    const beforeUpload = file => {
      ...
      this.props.actions.getFantasyAssistantUrl(data).then(() => {
        fetch(this.props.getFantasyAssistantUrlResponse.uploadUrl, {
          body: file,
          method: 'PUT'
        }).then(result => {
          if (result.status === 200) {
            this.setState({
              imageObject: this.props.getFantasyAssistantUrlResponse.object,
              loading: false,
              file
            });
            this.returnToParent();
...
function mapStateToProps(state, ownProps) {
  return {
    ...ownProps,
    getFantasyAssistantUrlResponse:
      state.superteamCricketFeed.getFantasyAssistantUrlResponse
  };
}
...
export default connect(mapStateToProps, mapDispatchToProps)(UploadSegment);
