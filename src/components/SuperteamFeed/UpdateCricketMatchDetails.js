...
class UpdateCricketMatchDetails extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      seasonGameUid: null
  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.props.actions.updateCricketFeedMatchDetail(data).then(() => {
          if (
            this.props.updateCricketFeedMatchDetailResponse &&
            this.props.updateCricketFeedMatchDetailResponse.error
          ) {
            message.error(
              this.props.updateCricketFeedMatchDetailResponse.error.message
                ? this.props.updateCricketFeedMatchDetailResponse.error.message
                : 'Could not update'
            );
          } else {
            this.props.history.push('/superteam-cricket-feeds/search');
          }
        });
      }
    });
...
function mapStateToProps(state, ownProps) {
  return {
    feedMatchDetails: state.superteamCricketFeed.feedMatchDetails,
    updateCricketFeedMatchDetailResponse:
      state.superteamCricketFeed.updateCricketFeedMatchDetailResponse
  };
}
...
const UpdateCricketMatchDetailsForm = Form.create()(UpdateCricketMatchDetails);
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(UpdateCricketMatchDetailsForm);
