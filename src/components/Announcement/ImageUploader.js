...
class ImageUploader extends React.Component {
  ...
  render() {
    ...
    const beforeUpload = file => {
      ...
      this.props.actions.getReferralConfigUploadUrl(imageData).then(() => {
        fetch(this.props.referralConfigUploadResponse.uploadUrl, {
          body: file,
          method: 'PUT'
        }).then(result => {
          if (result.status === 200) {
            this.setState({
              imageObject: this.props.referralConfigUploadResponse.object,
              loading: false,
              file
            });
            this.returnToParent();
...
function mapStateToProps(state, ownProps) {
  return {
    ...ownProps,
    state: state,
    referralConfigUploadResponse:
      state.referralConfig.referralConfigUploadResponse
  };
}
