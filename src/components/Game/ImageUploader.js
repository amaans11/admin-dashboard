...
class ImageUploader extends React.Component {
  ...
  render() {
    ...
    const beforeUpload = file => {
      ...
      this.props.actions.getDealUploadUrl(imageData).then(() => {
        fetch(this.props.deal.dealUploadAssetResponse.uploadUrl, {
          body: file,
          method: 'PUT'
        }).then(result => {
          if (result.status === 200) {
            this.setState({
              imageObject: this.props.deal.dealUploadAssetResponse.object,
              loading: false,
              file
            });
...
function mapStateToProps(state, ownProps) {
  return { ...ownProps, state: state, deal: state.deal };
}
