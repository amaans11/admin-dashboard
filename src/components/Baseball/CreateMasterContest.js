...
class CreateMasterContest extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      matchId: null,
    ...
    if (this.props.contestData) {
      ...
      this.props.form.setFieldsValue({
        ...
        appVersion: this.props.contestData.record.appVersion
          ...
          : 0,
        masterType: this.props.contestData.record.masterType
          ? this.props.contestData.record.masterType
          : '',
        bonusUtilization:
      ...
      if (this.props.contestData.actionType !== 'EDIT') {
        this.props.form.setFieldsValue({
          isGuaranteed: false
        });
        ...
        this.props.form.setFieldsValue({
          isGuaranteed: this.props.contestData.record.isGuaranteed
            ? true
            : false
        });
        this.setState({
      ...
      if (this.props.contestData.record.extraInfo) {
        ...
        if (
          JSON.parse(this.props.contestData.record.extraInfo).rakeBackDetails &&
          JSON.parse(this.props.contestData.record.extraInfo).rakeBackDetails
            .isRakeBack
        ) {
          this.setState({ isRakeBack: true }, () => {
            this.props.form.setFieldsValue({
              isRakeBack: this.state.isRakeBack,
              rakeBackMargin: JSON.parse(
                this.props.contestData.record.extraInfo
              ).rakeBackDetails.margin
            });
          });
        }
  ...
  componentWillUnmount() {
    this.props.actions.clearMasterContestForm();
  }
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (this.state.isRakeBack) {
          values.extraInfo['rakeBackDetails'] = {
            isRakeBack: this.state.isRakeBack,
            margin: values.rakeBackMargin
          };
        }
        ...
        if (this.state.actionType && this.state.actionType === 'EDIT') {
          ...
          this.props.actions.editMasterMatchContest(data).then(() => {
            if (
              this.props.fantasy.editMatchMasterContestResponse &&
              this.props.fantasy.editMatchMasterContestResponse.error
            ) {
              if (
                this.props.fantasy.editMatchMasterContestResponse.error.message
              ) {
                message.error(
                  this.props.fantasy.editMatchMasterContestResponse.error
                    .message
              ...
              this.props.history.push('/baseball/master-list');
            }
          });
        } else {
          this.props.actions.createMasterMatchContest(data).then(() => {
            if (
              this.props.fantasy.createMatchMasterContestResponse &&
              this.props.fantasy.createMatchMasterContestResponse.error
            ) {
              if (
                this.props.fantasy.createMatchMasterContestResponse.error
                  .message
  ...
  render() {
    ...
    const errors = {
      ...
      masterType: isFieldTouched('masterType') && getFieldError('masterType'),
      isGuaranteed:
      ...
      rakeBackMargin:
        isFieldTouched('rakeBackMargin') && getFieldError('rakeBackMargin'),
      ...
      <React.Fragment>
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          >
            ...
            <FormItem
              validateStatus={errors.masterType ? 'error' : ''}
              help={errors.masterType || ''}
              {...formItemLayout}
              label={
                <span>
                  Master Type
                  <Tooltip title="Master Type">
                    <Icon type="question-circle-o" />
                  </Tooltip>
                </span>
              }
            >
              {getFieldDecorator('masterType', {
                rules: [
                  {
                    required: true,
                    message: 'Please input master type!',
                    whitespace: true
                  }
                ]
              })(<Input />)}
            ...
            <FormItem
              ...
              label={
                <span>
                  Order Id
                  <Tooltip title="Order id for listing">
                    <Icon type="question-circle-o" />
                  </Tooltip>
                </span>
              }
            ...
            {this.state.isRakeBack && (
              <FormItem
                validateStatus={errors.rakeBackMargin ? 'error' : ''}
                help={errors.rakeBackMargin || ''}
                {...formItemLayout}
                label={
                  <span>
                    Rake Back Margin
                    <Tooltip title="Rake back maargin">
                      <Icon type="question-circle-o" />
                    </Tooltip>
                  </span>
                }
              >
                {getFieldDecorator('rakeBackMargin', {
                  rules: [
                    {
                      required: true,
                      type: 'number',
...
function mapStateToProps(state, ownProps) {
  return {
    allLineFormats: state.baseball.lineFormats,
    tierList: state.baseball.tierList,
    contestData: state.baseball.contestMasterData,
    ...
    getAllContestCategoryResponse: state.baseball.getAllContestCategoryResponse,
    getMlModelListResponse: state.baseball.getMlModelListResponse
  };
}
...
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators(
      { ...baseballActions, ...storageActions, ...superteamLeaderboardActions },
      dispatch
    )
  };
}
const CreateMasterContestForm = Form.create()(CreateMasterContest);
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(CreateMasterContestForm);
