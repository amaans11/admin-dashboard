        ...
        render: (text, record) => (
          <span>
            <Button
              size="small"
              type="primary"
              onClick={() => this.showPlayerScorePoints(record, 'admin')}
            >
              Details
            </Button>
            <Button
            ...
            {this.props.tabType === 'LIVE' && !matchDetail.autoFinish && (
              <Button
                icon="edit"
                size="small"
                onClick={() => this.openPlayerScoreEditModal(record)}
                type="primary"
              />
            )}
            ...
            <Col span={24}>
              ...
              <Col span={6}>
                {matchDetail.isInningsMatch && matchDetail.inningNumber && (
                  <Tag color="#2db7f5">Innings: {matchDetail.inningNumber}</Tag>
                )}
              ...
              <Col span={6}>
                ...
                JSON.parse(matchDetail.extraInfo).rakeBackDetails.isRakeBack ? (
                  <Tag color="green">RAKE BACK ENABLED</Tag>
                ) : (
                  <Tag color="red">RAKE BACK DISABlED</Tag>
                )}
              </Col>
              <Col span={6}>
                <Button onClick={() => this.getNotifications(matchDetail)}>
                  Edit Notification
                </Button>
              </Col>
            </Col>
...
function mapStateToProps(state, ownProps) {
  return {
    ...ownProps,
    contests: state.baseball.contests,
    fantasy: state.baseball,
    matchList: state.baseball.matches,
    getMatchPlayerScoreResponse: state.baseball.getMatchPlayerScoreResponse,
    initiatePrizeDistributionResponse:
      state.baseball.initiatePrizeDistributionResponse,
    getAllMasterContestTypeResponse:
      state.baseball.getAllMasterContestTypeResponse,
    createDefaultContestResponse: state.baseball.createDefaultContestResponse,
    updateMultiContestDetailResponse:
      state.baseball.updateMultiContestDetailResponse,
    updatePlayerScoreDetailResponse:
      state.baseball.updatePlayerScoreDetailResponse,
    refundMatchResponse: state.baseball.refundMatchResponse,
    cancelMatchResponse: state.baseball.cancelMatchResponse,
    currentUser: state.auth.currentUser,
    getAllSegmentTypeResponse: state.baseball.getAllSegmentTypeResponse,
    getMatchNotificationDetailResponse:
      state.baseball.getMatchNotificationDetailResponse,
    editMatchNotficationResponse: state.baseball.editMatchNotficationResponse,
    sendMatchNotficationManuallyResponse:
      state.baseball.sendMatchNotficationManuallyResponse,
    updateAllContestsForMatchResponse:
      state.baseball.updateAllContestsForMatchResponse,
    getMatchLevelMoneyResponse:
      state.superteamLeaderboard.getMatchLevelMoneyResponse,
    getContestDetailByIdResponse: state.baseball.getContestDetailByIdResponse,
    getContestLevelMoneyResponse:
      state.superteamLeaderboard.getContestLevelMoneyResponse,
    creditContestWinningsResponse: state.baseball.creditContestWinningsResponse,
    getContestMlPriceResponse: state.baseball.getContestMlPriceResponse,
    setContestMlPriceResponse: state.baseball.setContestMlPriceResponse,
    moveMatchFromLiveToUpcomingResponse:
      state.baseball.moveMatchFromLiveToUpcomingResponse
  };
}
...
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators(
      { ...baseballActions, ...superteamLeaderboardActions },
      dispatch
    )
  };
}
