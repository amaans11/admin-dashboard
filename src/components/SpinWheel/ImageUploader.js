...
class ImageUploader extends React.Component {
  ...
  render() {
    ...
    const beforeUpload = file => {
      ...
      this.props.actions.getWheelUploadAssetsUrl(imageData).then(() => {
        fetch(this.props.banner.wheelAssetUrl.uploadUrl, {
          body: file,
          method: 'PUT'
        }).then(result => {
          if (result.status === 200) {
            this.setState({
              imageObject: this.props.banner.wheelAssetUrl.object,
              loading: false,
              file
            });
...
function mapStateToProps(state, ownProps) {
  return { ...ownProps, state: state, banner: state.banner };
}
