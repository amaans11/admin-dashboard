  ...
  getSegments() {
    ...
    this.props.actions.getStSegmentList(data).then(() => {
      if (
        ...
        console.log('segment', segmentList);
        ...
        if (this.state.actionType && this.state.actionType === 'EDIT') {
          ...
          this.props.actions.editMasterMatchContest(data).then(() => {
            if (
              ...
              this.props.history.push('/fantasy/master-list');
            }
          });
        } else {
      ...
      this.props.actions.getFantasyImageUploadUrl(imageData).then(() => {
        fetch(this.props.fantasy.assetUrl.uploadUrl, {
          ...
          if (result.status === 200) {
            message.destroy();
            this.setState({
              imageUrl: this.props.fantasy.assetUrl.object.id,
              loading: false,
              file
            });
          }
...
function mapStateToProps(state, ownProps) {
  return {
    ...
    tierList: state.fantasy.tierList,
    contestData: state.fantasy.contestMasterData,
    fantasy: state.fantasy,
    getStSegmentListResponse:
      state.superteamLeaderboard.getStSegmentListResponse,
    getAllContestCategoryResponse: state.fantasy.getAllContestCategoryResponse,
    getMlModelListResponse: state.fantasy.getMlModelListResponse
  };
}
...
function mapDispatchToProps(dispatch) {
  return {
    actions: bindActionCreators(
      { ...fantasyActions, ...storageActions, ...superteamLeaderboardActions },
      dispatch
    )
  };
}
