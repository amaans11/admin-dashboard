...
class CreateMatch extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      fantasyAssistantAvailable: false,
      ...
      this.setState(
        {
          ...
          fantasyAssistantAvailable:
            record.extraInfo &&
            JSON.parse(record.extraInfo).fantasyAssistantAvailable
              ? JSON.parse(record.extraInfo).fantasyAssistantAvailable
              : false,
          fantasyAssistantInternalUrl:
            record.extraInfo &&
            JSON.parse(record.extraInfo).fantasyAssistantInternalUrl
              ? JSON.parse(record.extraInfo).fantasyAssistantInternalUrl
              : '',
          fantasyAssistantMatchId:
            record.extraInfo &&
            JSON.parse(record.extraInfo).fantasyAssistantMatchId
              ? JSON.parse(record.extraInfo).fantasyAssistantMatchId
              : ''
        },
        () => {
          this.props.form.setFieldsValue(
            {
              ...
              quickJoinEnabled:
                ...
                JSON.parse(record.extraInfo).quickJoinEnabled
                  ...
                  : false,
              // megaContestTag:
              //   record.extraInfo && JSON.parse(record.extraInfo).megaContestTag
              //     ? JSON.parse(record.extraInfo).megaContestTag
              //     : null,
              // megaContestPrize:
              //   record.extraInfo &&
              //   JSON.parse(record.extraInfo).megaContestPrize
              //     ? JSON.parse(record.extraInfo).megaContestPrize
              //     : null,
              ...
              seasonPassApplicables:
                ...
                JSON.parse(record.extraInfo).seasonPassApplicables
                  ? JSON.parse(record.extraInfo).seasonPassApplicables
                  : [],
              fantasyAssistantAvailable: record.fantasyAssistantAvailable
                ? record.fantasyAssistantAvailable
                : false
            },
            () => {
  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (this.state.fantasyAssistantAvailable !== '') {
          extraInfo[
            'fantasyAssistantAvailable'
          ] = this.state.fantasyAssistantAvailable;
        }
        if (
          this.state.fantasyAssistantMatchId &&
          this.state.fantasyAssistantMatchId !== ''
        ) {
          extraInfo[
            'fantasyAssistantMatchId'
        ...
        if (
          this.state.fantasyAssistantInternalUrl &&
          this.state.fantasyAssistantInternalUrl !== ''
        ) {
          extraInfo[
            'fantasyAssistantInternalUrl'
        ...
        if (this.state.editFlag) {
          this.props.actions.editMatchConfigDetails(data).then(() => {
            if (
              ...
              this.props.history.push('/fantasy/match-list');
            }
          });
        } else {
  ...
  getAllSeasonPass() {
    this.props.actions.getAllSeasonPass({ sportId: 7 }).then(() => {
      if (
  ...
  handleFantasyAssistantChange(e) {
    this.setState({
      fantasyAssistantAvailable: e.target.checked
    });
  }
  changefantasyAssistantInternalUrl(e) {
    this.setState({
      fantasyAssistantInternalUrl: e.target.value
    });
  }
        ...
        <Form onSubmit={this.handleSubmit}>
          <Card title="Create Match Config">
            ...
            {this.state.editFlag && (
              <FormItem
                {...formItemLayout}
                label={<span>Fantasy Assistant </span>}
              >
                <Checkbox
                  checked={this.state.fantasyAssistantAvailable}
                  onChange={e => this.handleFantasyAssistantChange(e)}
                />
              </FormItem>
            )}
            {this.state.fantasyAssistantAvailable && (
              <FormItem
                {...formItemLayout}
                label={<span>Assistant Options</span>}
              >
                <Card style={{ whiteSpace: 'pre-wrap' }} bordered={true}>
                  <p style={{ textAlign: 'center' }}>
                    {this.state.fantasyAssistantMatchId != '' &&
                    this.state.fantasyAssistantMatchId != 0
                      ? `Assistant MatchId : ${this.state.fantasyAssistantMatchId}`
                      : 'Please Attach Match Assistant'}
                  </p>
                  <p style={{ textAlign: 'center' }}>OR</p>
                  <span style={{ textAlign: 'center' }}>
                    Fantasy Assistant Url :
                  </span>
                  <Input
                    required={
                      this.state.fantasyAssistantMatchId != '' &&
                    ...
                    value={this.state.fantasyAssistantInternalUrl}
                    onChange={e => this.changefantasyAssistantInternalUrl(e)}
                    style={{ width: '70%' }}
                  ></Input>
                </Card>
              </FormItem>
...
function mapStateToProps(state, ownProps) {
  return {
    ...
    allLineFormats: state.fantasy.lineFormats,
    matchConfig: state.fantasy.matchConfig,
    ...
    getAllSeasonPassResponse: state.fantasy.getAllSeasonPassResponse,
    getCdnPathForUploadResponse: state.website.getCdnPathForUploadResponse
    // getAllCountryCodeResponse: state.fantasy.getAllCountryCodeResponse
  };
}
