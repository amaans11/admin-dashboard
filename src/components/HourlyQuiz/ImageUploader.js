...
class ImageUploader extends React.Component {
  ...
  render() {
    ...
    const beforeUpload = file => {
      ...
      this.props.actions.getHourlyQuizUrl(imageData).then(() => {
        fetch(this.props.state.hourlyQuiz.hourlyQuiz.uploadUrl, {
          body: file,
          method: 'PUT'
        }).then(result => {
          if (result.status === 200) {
            this.setState({
              imageObject: this.props.state.hourlyQuiz.hourlyQuiz.object,
              loading: false,
              file
            });
            this.returnToParent();
